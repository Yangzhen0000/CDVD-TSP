...
class C3D(nn.Module):
    def __init__(self, in_channels=3, n_sequence=3, out_channels=3, n_resblock=3, n_feat=32, device='cuda'):
        ...
        Encoder_first = [nn.Sequential(
            nn.Conv3d(n_feat, n_feat * 2, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1)),
            nn.ReLU(inplace=True)
        )]
        ...
        Encoder_second = [nn.Sequential(
            nn.Conv3d(n_feat * 2, n_feat * 4, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1)),
            nn.ReLU(inplace=True)
        )]
        ...
        Decoder_second.append(nn.Sequential(
            nn.ConvTranspose3d(n_feat * 4, n_feat * 2, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1), output_padding=(1, 1, 1)),
            nn.ReLU(inplace=True)
        ))
        # decoder1
        ...
        Decoder_first.append(nn.Sequential(
            nn.ConvTranspose3d(n_feat * 2, n_feat, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1), output_padding=(1, 1, 1)),
            nn.ReLU(inplace=True)
        ))
    ...
    def forward(self, x):
        in_sequence = x.permute(0, 2, 1, 3, 4)

        inblock = self.inBlock(in_sequence)
        encoder_first = self.encoder_first(inblock)
        encoder_second = self.encoder_second(encoder_first)
